...
def today_window() -> tuple[int, int]:
    """
    Wide UTC window covering 'today' for every timezone (UTC-12…UTC+14).
    Returns a 52-hour window: 14h before UTC midnight … 14h after UTC midnight+1d.
    Per-user filtering by local date is done in the handler after fetching.
    """
    now   = _utc_now()
    start = now.replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(hours=14)
    end   = start + timedelta(hours=52)
